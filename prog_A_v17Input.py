...
if __name__ == '__main__': #multiprocessing imports script as module
    ...
    inten_high: %s \n inten_low: %s \n Temp: %s \n Jmax: %s \n freq_uncertainty: %s \n number of triples: %s \n Check peaks:\n%s \n trans_1: %s \n trans_2: %s \n trans_3: %s "%(job_name,u_A,u_B,u_C,A,B,C,DJ,DJK,DK,dJ,dK,str(processors),str(freq_high),\
        ...
        for entry in peaklist:
            peaklist_str+=str(entry[0]) + " " + str(entry[1])+"\n"
            
        trans_1_str = ""
        for entry in trans_1:
            trans_1_str+=str(entry)+"\n"
            
        trans_2_str = ""
        for entry in trans_2:
            trans_2_str+=str(entry)+"\n"
            
        trans_3_str = ""
        for entry in trans_3:
            trans_3_str+=str(entry)+"\n"
            
        trans_2_peak_str = ""
        for entry in trans_2_peaks:
            trans_2_peak_str+=str(entry)+"\n"
            
        trans_3_peak_str = ""
        for entry in trans_3_peaks:
            trans_3_peak_str+=str(entry)+"\n"
        

        
        num_sever=input("Choose the number of server: ")
        ...
        num_div = num_sever+1
        core_server=raw_input("Enter the number of server core: ")
        
#        core = []
#        hosts= []
#        
#        for num in range(num_sever):
#            host=raw_input("Enter the server address: ")
#            hosts.append(host)
#            core_server=raw_input("Enter the number of server core: ")
#            core.append(core_server)
        
        start=0
        stop = len(trans_1_peaks)/num_div

        for num in range(num_div):
            trans_1_peak_str = ""
            for entry in trans_1_peaks[start:stop]:
                trans_1_peak_str+=str(entry)+"\n"
            
            trans_file = ""
            trans_file += "A: %s \nB: %s \nC: %s \nDJ: %s \nDJK: %s \nDK: %s \ndJ: %s \ndK: %s \nprocessors: %s \nfolder: %s \nnum_server: %s \ntrans_1:\n%s \ntrans_2:\n%s \ntrans_3:\n%s \ntrans_1_peaks:\n%s \ntrans_2_peaks:\n%s \ntrans_3_peaks:\n%s \ntop_peaks_3cut:\n%s \npeaklist:\n%s"%(A,B,C,DJ,DJK,DK,dJ,dK,\
            str(core_server),job_name,num_sever,trans_1_str,trans_2_str,trans_3_str,trans_1_peak_str,trans_2_peak_str,trans_3_peak_str,fitting_peaks_str, peaklist_str)
            file_send  = open("input_parameters%s.txt"%(num),"w")
            file_send.write(trans_file) 
            file_send.close()
            
            start=stop
            stop=stop + len(trans_1_peaks)/num_div
            if num_sever==1:
                stop=len(trans_1_peaks)
